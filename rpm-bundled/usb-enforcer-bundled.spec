...
if [ $1 -eq 2 ]; then
    ...
    if ! grep -q "allow_plaintext_write_with_scanning" %{_sysconfdir}/usb-enforcer/config.toml 2>/dev/null; then
        sed -i '/allow_luks1_readonly = true/a allow_plaintext_write_with_scanning = false  # Allow write to unencrypted drives if content scanning is enabled' \
            %{_sysconfdir}/usb-enforcer/config.toml 2>/dev/null || true
    fi
fi
